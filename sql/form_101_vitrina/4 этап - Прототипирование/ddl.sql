-- Исправление данных в источнике

drop table if exists bank.f101_mart;

-- DDL: создание таблицы витрины
create table if not exists bank.f101_mart (
    regn          integer not null,
    plan          text    not null,
    num_sc        varchar(5) not null,
    a_p           char(1) not null,
    vitg_rub      integer not null default 0,
    vitg_currency integer not null default 0,
    vitg_total    integer not null default 0,
    oitga_rub     integer not null default 0,
    oitga_currency integer not null default 0,
    oitga_total   integer not null default 0,
    oitgp_rub     integer not null default 0,
    oitgp_currency integer not null default 0,
    oitgp_total   integer not null default 0,
    iitg_rub      integer not null default 0,
    iitg_currency integer not null default 0,
    iitg_total    integer not null default 0,
    dt            date    not null,
    constraint pk_f101_mart primary key (regn, plan, num_sc, dt)
);

-- Наполнение витрины данными (финальный прототип)
-- все суммы в тыс. рублей, округлены до целых
insert into bank.f101_mart
with
